...
let UtfGrid;
const initUtfGrid = () => {
  UtfGrid = class UtfGridInternal extends globalThis.google.maps.OverlayView {
    constructor(url, options) {
      ...
      this.on = function(event, handler) {
        var handle = globalThis.google.maps.event.addListener(self, event, handler);
        handles.push(handle);
      };
      this.off = function() {
        while (handles.length) {
          globalThis.google.maps.event.removeListener(handles.pop());
        }
      };
    }
    onAdd() {
    ...
    _removeHandlers(all) {
      if (all && this._zoomEvent) {
        globalThis.google.maps.event.removeListener(this._zoomEvent);
    ...
    _click(e) {
      this._move(e);
      if (this._mouseOn) {
        globalThis.google.maps.event.trigger(this, 'click', {
          latLng: e.latLng,
          data: this._mouseOn
        });
      }
    ...
    _move(e) {
      ...
      if (!equal3Array(on.data, this._mouseOn)) {
        if (this._mouseOn) {
          globalThis.google.maps.event.trigger(this, 'mouseout', { latLng: e.latLng, data: this._mouseOn });
        ...
        if (on.data) {
          ...
          globalThis.google.maps.event.trigger(this, 'mouseover', on);
    ...
    _getPixelCoordinates(coord) {
      ...
      var pixelCoordinate = new globalThis.google.maps.Point(
    ...
    _loadTile(zoom, x, y) {
      ...
      this._url(z, xTile, yTile).then(url => {
        ...
        var fullUrl = url[0] + '?' + stringify(url[1]);
        if (this._reqCache.has(fullUrl)) {
          return this._reqCache.get(fullUrl);
        }
        return getJson(fullUrl).then(data => {
    ...
    _decodeTile(gridX, gridY, gridTile) {
      ...
      if (dataIndex !== -1) {
        let out = gridTile.data.data[dataIndex];
        if (out) {
          return out;
        }
      }
      ...
      for (let change of tests) {
        gridIndex += change;
        if (gridIndex > len) {
          continue;
        }
        if (gridTile.data.index[gridIndex] !== -1) {
          let out = gridTile.data.data[gridTile.data.index[gridIndex]]
          if (out) {
            return out;
          }
        }
...
export default (url, options)=> {
  ...
  return new UtfGrid(url, options);
}
