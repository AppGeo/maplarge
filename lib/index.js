...
export default class Layer extends EE.EventEmitter {
  ...
  removeFromOverlayMapTypes() {
    ...
    let index = this.map.overlayMapTypes.indexOf(this.tileLayer);
    while (index > -1) {
  ...
  getInfo(lat, lon, zoom) {
    ...
    return this.cacheRequest(data, 'ProcessDirect').then(resp => this.transposeResp(resp));
  }
  getFields(field) {
    var data = {
      uParams: 'action:table%2Fquery',
  ...
  checkRegistration(json, hash) {
    ...
    var url = `https://${~~(Math.random() * 3)}${this.host}/Api/VerifyRequest`;
    var opts = {
      hash: hash
    };
  ...
  addTo(map) {
    ...
    if (!this.tileLayer) {
      this.tileLayer = new google.maps.ImageMapType({
        getTileUrl: () => `https://maps.gstatic.com/mapfiles/transparent.png`,
        tileSize: new google.maps.Size(256, 256),
        minZoom: this.minZoom,
        maxZoom: this.maxZoom
      });
      this.tileLayer.getTile = (coord, zoom, doc) => {
        var out;
        if (divs.length) {
          out = divs.pop();
        } else {
        ...
        this.divCache.set(out, {
          coord,
          zoom
        });
        this.getTileUrl(zoom, coord.x, coord.y).then(url => {
          if (this.replaceImg(url, out) && this.utfGrid) {
            this.utfGrid._loadTile(zoom, coord.x, coord.y);
          }
        });
        return out;
      };
      this.tileLayer.releaseTile = (div) => {
        this.divCache.delete(div);
        this.waitingImages.delete(div);
        if (div.firstChild) {
